...
在本节中，我们将介绍两种最常见的OAuth授权类型的基础知识。如果你完全不熟悉OAuth，我们建议在尝试完成[OAuth认证](https://portswigger.net/web-security/oauth)实验前先阅读本节内容。
...
对于基本的OAuth，客户端应用可以请求访问的作用域对于每个OAuth服务都是唯一的。由于作用域的名称只是一个任意文本字符串，因此其格式在不同的提供者之间会有很大差异。有些甚至使用一个完整的URI作为作用域的名称，类似于REST API的端点。例如，当请求对用户联系人列表的读取访问权限时，根据所使用的OAuth服务，作用域名称可能采用以下任何一种形式：
...
授权码授权类型一开始看起来非常复杂，但一旦熟悉了一些基础知识，它实际上比你想象的要简单。
...
除了`client_id`和授权`code`外，还会注意到以下新的参数：
